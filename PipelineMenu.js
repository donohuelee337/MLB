...
function onOpen() {
  ...
  ui.createMenu('⚾ MLB-BOIZ')
    ...
    .addItem('📣 Batter Grand Slam promo sheet (scaled λ, no odds)', 'refreshBatterGsPromoSheet_')
...
function runMLBBallWindow_(windowTag, skipInjuriesFetch) {
  ...
  step('Batter HR model (P(HR≥1))', refreshBatterHRQueue);
  step('Batter HR promo sheet', refreshBatterHrPromoSheet_);
  step('Batter Grand Slam promo sheet', refreshBatterGsPromoSheet_);
  ...
  const oBbWalkQ = outcomes[11] || { ok: true };
  const oBbWalkC = outcomes[12] || { ok: true };
  ...
  const oGsPromo = outcomes[23] || { ok: true };
  const oBet = outcomes[24] || { ok: true };
  ...
  logStep_(
    'Pitcher Walks queue',
    0,
    oBbWalkQ.ok ? mlbTabDataRowsBelowHeader3_(ss, MLB_PITCHER_BB_QUEUE_TAB) : 0,
    oBbWalkQ.ok ? '' : oBbWalkQ.err || 'failed'
  );
  logStep_(
    'Pitcher Walks card',
    0,
    oBbWalkC.ok ? mlbTabDataRowsBelowHeader3_(ss, MLB_PITCHER_BB_CARD_TAB) : 0,
    oBbWalkC.ok ? '' : oBbWalkC.err || 'failed'
  );
  logStep_(
    'Pitcher HA queue',
    0,
  ...
  logStep_(
    'Batter HR model',
    0,
    oHrModel.ok ? mlbTabDataRowsBelowHeader3_(ss, MLB_BATTER_HR_MODEL_TAB) : 0,
    oHrModel.ok ? '' : oHrModel.err || 'failed'
  );
  logStep_(
    'Batter HR promo',
    0,
  ...
  logStep_(
    'Batter GS promo',
    0,
    oGsPromo.ok ? mlbTabDataRowsBelowHeader3_(ss, '📣 Batter_GS_Promo') : 0,
    oGsPromo.ok ? '' : oGsPromo.err || 'failed'
  );
  logStep_(
    'MLB Bet Card',
    0,
