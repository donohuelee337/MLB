...
function mlbHittingHitsSummary_(playerId, season) {
  ...
  const l5Avg = l5n > 0 ? Math.round((l5h / l5n) * 1000) / 1000 : '';
  // Hot/Cold requires a meaningful season baseline (≥10 games) and a full L5 window.
  const hotCold = n >= 10 && l5n >= 5 ? mlbHotColdFlag_(l5Avg, hpgSzn) : '';
  return {
    hpgSzn: hpgSzn,
    l7h: l7n ? l7h : '',
    l7n: l7n || '',
...
function refreshBatterHitsSlateQueue() {
  ...
  [72, 200, 150, 88, 56, 64, 64, 64, 44, 52, 220, 88, 140, 160, 64, 56].forEach(function (w, i) {
  ...
  sh.getRange(1, 1, 1, 16)
    .merge()
    .setValue(
      '📋 Batter hits queue — FD batter_hits + hitting gameLog (L5/L7/season H·game + hot/cold) · ' + season
    )
    .setFontWeight('bold')
