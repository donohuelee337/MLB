...
/**
 * Plate appearances from statsapi boxscore batting line (0 if listed but no PA field).
 * Used to pick the correct gamePk on doubleheaders when odds↔schedule map is ambiguous.
 */
function mlbBatterPlateAppearancesFromBoxscore_(payload, batterId) {
  const teams = mlbBoxscoreTeams_(payload);
  if (!teams) return null;
  const pid = 'ID' + parseInt(batterId, 10);
  const sides = ['away', 'home'];
  for (let s = 0; s < sides.length; s++) {
    ...
    const bat = pl && pl.stats && pl.stats.batting;
    if (!bat) continue;
    if (bat.plateAppearances != null && String(bat.plateAppearances).trim() !== '') {
      return parseInt(bat.plateAppearances, 10) || 0;
    }
    const ab = bat.atBats != null ? parseInt(bat.atBats, 10) || 0 : 0;
    const bb = bat.baseOnBalls != null ? parseInt(bat.baseOnBalls, 10) || 0 : 0;
    const hbp = bat.hitByPitch != null ? parseInt(bat.hitByPitch, 10) || 0 : 0;
    const sf = bat.sacFlies != null ? parseInt(bat.sacFlies, 10) || 0 : 0;
    const sh = bat.sacBunts != null ? parseInt(bat.sacBunts, 10) || 0 : 0;
    return ab + bb + hbp + sf + sh;
  }
  return null;
}

/** All statsapi gamePk for this slate date whose matchup normalizes to matchupStr (e.g. doubleheaders). */
function mlbScheduleAllGamePksForMatchup_(slateYmd, matchupStr) {
  ...
  for (let d = 0; d < dates.length; d++) {
    ...
    for (let i = 0; i < games.length; i++) {
      ...
      const m = (away.name || '') + ' @ ' + (home.name || '');
      if (mlbNormalizeGameLabel_(m) !== wantG) continue;
      const pk = parseInt(g.gamePk, 10);
      if (!pk || isNaN(pk) || seen[pk]) continue;
      seen[pk] = true;
      out.push(pk);
    }
  }
...
/**
 * When two games share the same matchup label, odds→gamePk can point at the wrong contest.
 * Among FINAL boxscores, pick the game where this batter logged the most PA (tie → keep tentativePk).
 * @returns {{ gamePk: number, box: Object|null }} box is the winning game's payload when disambiguated.
 */
function mlbDisambiguateBatterGamePkAndMaybeBox_(slateStr, matchup, batterId, tentativePk) {
  const tid = parseInt(tentativePk, 10);
  const bid = parseInt(batterId, 10);
  if (!slateStr || !matchup || !tid || isNaN(tid) || !bid || isNaN(bid)) {
    return { gamePk: tentativePk, box: null };
  }
  const cand = mlbScheduleAllGamePksForMatchup_(slateStr, matchup);
  if (cand.length <= 1) return { gamePk: tentativePk, box: null };
  ...
  for (let i = 0; i < cand.length; i++) {
    const pkTry = cand[i];
    const bx = mlbFetchBoxscoreJson_(pkTry);
    Utilities.sleep(120);
    if (!bx || !mlbBoxscoreIsFinal_(bx)) continue;
    const pa = mlbBatterPlateAppearancesFromBoxscore_(bx, bid);
    if (pa == null) continue;
    if (pa > bestPa || (pa === bestPa && pkTry === tid)) {
      bestPa = pa;
      bestPk = pkTry;
      bestBox = bx;
    }
  }
  if (bestPa < 0) return { gamePk: tentativePk, box: null };
  return { gamePk: bestPk, box: bestBox };
}
...
function gradeMLBPendingResults_() {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    // Doubleheader fix: same normalized matchup can map to the wrong gamePk; pick the FINAL game
    // where this batter logged the most PA before grading TB / hits / HR.
    let prefetchedBox = null;
    if ((isTb || isBatterHits || isHr) && slateStr && matchup) {
      const pick = mlbDisambiguateBatterGamePkAndMaybeBox_(slateStr, matchup, pid, gamePk);
      const g2 = parseInt(pick.gamePk, 10);
      if (!isNaN(g2) && g2 > 0) gamePk = g2;
      prefetchedBox = pick.box;
    }

    let box = prefetchedBox;
    if (!box) {
      box = mlbFetchBoxscoreJson_(gamePk);
      Utilities.sleep(120);
    }
    if (!box) {
