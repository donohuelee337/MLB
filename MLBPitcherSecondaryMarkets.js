...
function refreshPitcherOutsSlateQueue() {
  ...
  if (pack.err === 'no_schedule') {
    ss.toast('Run MLB schedule first.', 'Pitcher Outs queue', 6);
    return;
  }
...
function refreshPitcherWalksSlateQueue() {
  ...
  if (pack.err === 'no_schedule') {
    ss.toast('Run MLB schedule first.', 'Pitcher Walks queue', 6);
    return;
  }
  mlbWritePitcherSecondaryQueueSheet_(
    ss,
...
function refreshPitcherHitsAllowedSlateQueue() {
  ...
  if (pack.err === 'no_schedule') {
    ss.toast('Run MLB schedule first.', 'Pitcher HA queue', 6);
    return;
  }
  mlbWritePitcherSecondaryQueueSheet_(
    ss,
...
function mlbPitcherSecondaryBetCardBody_(queueTab, lambdaFn, meta) {
  ...
  if (!q || q.getLastRow() < 4) {
    ss.toast(meta.alertDetail || 'Queue empty — skipped.', meta.alertTitle || 'MLB-BOIZ', 6);
    return;
  }
