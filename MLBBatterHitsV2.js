...
function refreshBatterHitsV2BetCard() {
  ...
  // Default new sheets ship with 26 columns; this layout writes through col 34.
  // Cols 33-34 (hp_umpire, hot_cold) feed 🃏 MLB_Bet_Card now that v2 is the
  // live H source — see MLBBetCard.js H block reader.
  const NEED_COLS = 34;
  ...
  sh.getRange(1, 1, 1, 34)
    .merge()
    .setValue(
      '🎯 Batter Hits v2 (LIVE) — λ = H/PA(vs hand) × est_PA × park_H × opp_SP_H/9; feeds 🃏 MLB_Bet_Card'
    )
    .setFontWeight('bold')
