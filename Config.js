...
function buildConfigTab() {
  ...
  row_(
    'GS_PROMO_LEAGUE_GS_PER_HR',
    '0.027',
    'Rough league grand slams per home run (tune yearly). 📣 Batter_GS_Promo scales HR promo λ by this × optional order weights.'
  );
  row_(
    'GS_PROMO_ORDER_WEIGHT_JSON',
    '',
    'Optional JSON array length 9 — relative multipliers for batting orders 1..9 on 📣 Batter_GS_Promo. Blank = small built-in heart-of-order tilt.'
  );
  ...
  // Rebuild ⚾ MLB-BOIZ menu so new menu items appear without reloading the browser tab.
  try {
    if (typeof onOpen === 'function') onOpen();
  } catch (e) {
    Logger.log('buildConfigTab: onOpen menu refresh skipped: ' + e);
  }
}
...
function validateMlbPipelineConfig_(cfg) {
  ...
  warnRange('GS_PROMO_LEAGUE_GS_PER_HR', c['GS_PROMO_LEAGUE_GS_PER_HR'], 0.005, 0.08);
