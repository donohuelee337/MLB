...
function mlbHittingTbSummary_(playerId, season) {
  ...
  const l5Avg = l5n > 0 ? Math.round((l5tb / l5n) * 1000) / 1000 : '';
  const hotCold = n >= 10 && l5n >= 5 ? mlbHotColdFlag_(l5Avg, tbpgSzn) : '';
  return {
    tbpgSzn: tbpgSzn,
    l7tb: l7n ? l7tb : '',
    l7n: l7n || '',
...
function refreshBatterTbSlateQueue() {
  ...
  sh.getRange(1, 1, 1, 16)
    .merge()
    .setValue(
      '📋 Batter total bases queue — FD batter_total_bases + hitting gameLog (L5/L7/season TB·game + hot/cold) · ' + season
    )
    .setFontWeight('bold')
