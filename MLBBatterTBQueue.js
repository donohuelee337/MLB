...
/** Poisson FD prop cards (MLBBatterTBBetCard.js) — declared once to avoid duplicate const with legacy card files. */
...
function mlbCollectBatterOddsRows_(ss, marketKey) {
  ...
  const wantAlt = want ? want + '_alternate' : '';
  const byKey = {};
  ...
  for (let i = 0; i < block.length; i++) {
    ...
    if (market !== want && market !== wantAlt) continue;
