...
/**
 * Shadow tracker — same panel format, reads 🧪 MLB_Results_Log_v2 which mirrors
 * the live log's column layout for indices 0..23. Tracks whichever hits model
 * is currently shadowed (h.v1 since the h.v2-full promotion). Renders even
 * when log is missing so user sees the panel scaffolding.
 */
function mlbAppendBetTrackerSectionV2_(ss, sh, startRow, slateDate) {
  ...
  return _mlbRenderBetTrackerPanel_(ss, sh, startRow, slateDate, {
    ...
    title: 'Bet Tracker (shadow)  ·  h.v1 baseline model  ·  hits only',
    markets: [
      { key: 'H', label: 'HITS shadow', test: function (m) { return m.indexOf('batter hit') !== -1; } },
    ],
  });
}
...
function mlbApplyPropCardFormatting_(sh, rows, headers, opts) {
  ...
  const cLowPa = mlbPropCol_(headers, 'low_sample_pa');
  if (cLowPa > 0) {
    const cSeasonPa = mlbPropCol_(headers, 'season_pa');
    const cSeasonAb = mlbPropCol_(headers, 'season_ab');
    for (let i = 0; i < n; i++) {
      if (String(rows[i][cLowPa - 1] || '').toUpperCase() !== 'LOW') continue;
      sh.getRange(startRow + i, cLowPa, 1, 1)
        .setBackground('#ffebee')
        .setFontColor('#b71c1c')
        .setFontWeight('bold');
      if (cSeasonPa > 0) {
        sh.getRange(startRow + i, cSeasonPa, 1, 1).setBackground('#fff3e0').setFontColor('#e65100');
      }
      if (cSeasonAb > 0) {
        sh.getRange(startRow + i, cSeasonAb, 1, 1).setBackground('#fff3e0').setFontColor('#e65100');
      }
    }
  }
}
