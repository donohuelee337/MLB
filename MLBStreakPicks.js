...
const MLB_STREAK_MIN_PA_DEFAULT = 50;
...
function mlbStreakMinPa_(cfg) {
  const raw = String(cfg['STREAK_MIN_PA'] != null ? cfg['STREAK_MIN_PA'] : MLB_STREAK_MIN_PA_DEFAULT).trim();
  const n = parseInt(raw, 10);
  if (isNaN(n) || n < 0) return MLB_STREAK_MIN_PA_DEFAULT;
  return n;
}
...
function refreshStreakPicks() {
  ...
  const minPa = mlbStreakMinPa_(cfg);
  ...
  candidates.forEach(function (c) {
    ...
    let seasonAb = '';
    let lowSamplePa = '';
    if (c.batterId && typeof mlbSharedBatterSeasonPaAb_ === 'function') {
      const paAb = mlbSharedBatterSeasonPaAb_(c.batterId, season);
    ...
    if (lowSamplePa === 'LOW') notes.push('low_pa');
  ...
  const ranked = deduped.map(function (row) {
    ...
    let rank = '';
    let isPick = '';
    if (havePstreak && pickIdx < pickCount && row.lowSamplePa !== 'LOW') {
      pickIdx += 1;
      rank = pickIdx;
      isPick = true;
    }
...
function mlbStreakWriteSheet_(ss, rows, slateDate, hotColdFlags) {
  ...
  const widths = [72, 200, 150, 44, 72, 64, 72, 52, 52, 56, 130, 64, 64, 72, 80, 60, 72, 64, 72, 80, 72, 64, 56, 56, 80, 220];
...
function mlbStreakPicksByPlayer_(ss) {
  ...
  // Schema (1-indexed): col 3 = batter, col 22 = p_streak, col 23 = pick_rank,
  // col 24 = is_pick (season_pa/ab/low_sample_pa at 8–10).
  const vals = sh.getRange(4, 1, last - 3, 24).getValues();
  for (let i = 0; i < vals.length; i++) {
    if (vals[i][23] !== true) continue;
    const name = String(vals[i][2] || '').trim().toLowerCase();
    if (!name) continue;
    const rank = parseInt(vals[i][22], 10) || 0;
    const pStreak = parseFloat(vals[i][21]);
